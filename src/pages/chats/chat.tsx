...
const ChatPage: React.FC = () => {
  ...
  const { 
    ...
    setIsProcessing,
    initializeHistory,
  } = useLangChainService(systemContext, { baseUrl: 'http://localhost:11434' });
    ...
    const initializeChat = async () => {
      ...
      try {
        ...
        if (state?.existingChat && state?.chatId) {
          ...
          // Initialize new chat with proper type
  ...
  const handleChatCreated = async (chatId: string) => {
	// If this chat was created from a project context, associate it with the project
		if (projectContext?.projectId) {
			try {
				await projectService.addChat(projectContext.projectId, {
					chatId,
					includeInRAG: true,
					addedAt: new Date()
				});
