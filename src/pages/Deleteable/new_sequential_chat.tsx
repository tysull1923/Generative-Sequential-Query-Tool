...
import { ChatRequest, Role, ChatResponse } from '@/lib/api/openai.api-requests.types';
...
const NewChatPage = () => {
	...
	const [settings, setSettingsChange] = useState<ChatSettings>({
		temperature: .7,
						...
						{requests.map((request, index) => (
							request.type === 'pause' ? (
								...
								<ChatCard
									...
									response={String(request.response)} // Add this line
								// key={request.id}
								// {...request}
								// isFirst={index === 0}
								// isLast={index === requests.length - 1}
